...
## 1. 项目概述
### 1.1 项目背景
- 本项目是一个中文学习平台，旨在通过AI生成的故事故事帮助非中文母语的学习者提升中文水平。
- 项目由**蓝色冰川**（语言学习机构）发起，作为其大项目的一部分，核心目标是根据用户中文水平生成适合的故事。
- **蓝色冰川**的需求是根据词汇表和中文词汇的级别生成带有场景的故事，并控制故事中的生字率，以提升用户体验。

### 1.2 项目目标
- 通过生成符合用户中文水平的故事，帮助用户更好地掌握词汇和语法。
- 提升用户在特定场景下的语言应用能力。
- 控制故事中的生字率，以适应不同学习阶段用户的需求。
...
- **核心功能**：
  ...
  - 提示语生成：根据词汇级别、场景标签、篇幅、生字率等参数生成提示语。
- **数据格式**：提示语以文本格式存储，支持动态更新。

### 3.5 故事生成模块
- **功能描述**：通过DeepSeek API生成符合用户需求的故事。
- **核心功能**：
  - 故事生成：根据提示语生成故事，支持设置篇幅、生字率等参数。
  - 故事返回：生成的故事包含英文注释、生词列表和生字率，以JSON格式返回。
- **数据格式**：故事数据以JSON格式存储，包含故事ID、标题、内容、词汇级别、场景标签、生词列表、生字率等信息。

### 3.6 故事升级/降级模块
- **功能描述**：对现有故事进行升级或降级，调整词汇级别和生字率。
- **核心功能**：
  - 升级/降级逻辑：根据目标级别替换或添加词汇和生字。
...
- **核心功能**：
  - 故事生成API：支持设置篇幅、生字率、场景、目标词汇级别等参数。
  - 场景管理API：支持场景的CRUD操作。
...
### 4.4 提示语工程
- 提供多种提示语模板，用于生成不同场景和生字率的故事。
- 提示语示例：
  - "请根据XX场景，设计一个小故事（或对话），字数为300（正负30字），生字率为0.01（正负0.005），目的是让学习者掌握XXX级别的词汇（或XXX语法）。可选XXX词语为：（从JSON搜索出来的）"

### 4.5 故事生成
- 通过DeepSeek API生成故事，故事内容需符合用户指定的词汇级别、生字率和场景。
- 支持设置故事的篇幅（字数）、生字率和生字率容差。
- 生成的故事包含英文注释和生词列表，以及实际的生字率。
- 故事生成API返回的JSON格式如下：
  ```json
  {
...
### 4.6 故事升级/降级
- 支持对现有故事进行升级或降级，根据目标级别替换或添加词汇和生字。
- 升级/降级逻辑：
  - 从当前级别（如5级）升级到目标级别（如6级），替换或添加更高级别的词汇和生字。
  - 从当前级别（如5级）降级到目标级别（如4级），替换或添加更低级别的词汇和生字。
- 升级/降级API参数：
...
## 6. 用户流程
1. **蓝色冰川**通过API调用，传入参数（篇幅、篇幅容差、生字率、生字率容差、场景、目标词汇级别、生词数量）。
2. 后端根据参数查询词汇表、生字表和场景数据，生成提示语。
3. 调用DeepSeek API生成故事。
4. 返回生成的故事（JSON格式）给**蓝色冰川**。
5. **蓝色冰川**将故事提供给最终用户学习。
6. 用户或机构可以通过故事升级/降级API调整故事的词汇级别和生字率。
...
- **第4周**：开发故事生成API，支持场景标签、篇幅设置和生字率控制。
- **第5周**：开发故事升级/降级功能。
- **第6周**：测试与优化API性能。
