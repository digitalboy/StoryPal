...
- 分词工具: 使用自定义分词，基于数据库中的词汇表进行分词。
- 数据存储：使用 PostgreSQL 数据库存储词语表、场景数据和其他信息。
...
- **核心功能**：
  ...
  - 词语查询：根据级别、词性等条件查询词语。
  - 词语更新：支持动态更新词语表。
- **数据格式**：词语数据存储在 PostgreSQL 数据库中，详细格式见 `docs/data_models.md`。
...
- **核心功能**：
  ...
  - 场景查询：根据场景名称或描述查询场景。
  - 场景更新：支持动态更新场景数据。
- **数据格式**：场景数据存储在 PostgreSQL 数据库中，详细格式见 `docs/data_models.md`。
...
- **数据格式**：故事数据存储在 PostgreSQL 数据库中，详细格式见 `docs/data_models.md`。
...
- **第 1 周**：完成词语表和场景数据的设计与 PostgreSQL 数据库存储。
- **第 2 周**：开发词语查询和场景管理功能。
...
- **生字率的计算**: 生字率 = 生字数量 / 故事总字数, **同一个字在不同词性下视为不同词汇**。
  - **使用自定义分词逻辑，基于 PostgreSQL 数据库中的词汇表进行分词。**
