...
class Config:
    ...
    # 故事字数容差值
     ...
     # 加载词汇数据的路径
     BASE_DIR = os.path.dirname(os.path.abspath(__file__)) # 获取当前文件(config.py)的绝对路径
     WORDS_FILE_PATH = os.path.join(BASE_DIR, '..', os.getenv("WORDS_FILE_PATH", "app/data/words.json"))
     SCENES_FILE_PATH = os.path.join(BASE_DIR, '..', os.getenv("SCENES_FILE_PATH", "app/data/scenes.json"))
