...
*   **API 接口**: 测试 API 接口的请求参数验证、业务逻辑处理和响应格式是否正确，包括：
    *   故事生成 API： 验证请求参数，生字率计算，重点词汇，和 JSON 响应。
    *   场景管理 API：验证场景的 CRUD 操作。
    *   字词查询 API：验证分页查询和参数过滤。
...
*   **单元测试**:
    *   更新了测试范围，包括模型层、服务层、工具函数和提示语生成。
    *   添加了对 `characters` 字段的测试要求。
*   **集成测试**:
    *   详细描述了 API 接口的测试范围，包括故事生成、场景管理、字词查询和故事升级/降级 API。
*   **性能测试**:
    *   添加了具体的性能测试指标，包括 API 的响应时间和吞吐量。
*   **错误处理测试**:
    *   添加了数据验证失败的错误测试。
*  **最佳实践**:
     *   添加了测试代码独立性的要求。
*  **其他**:
    *   更新了测试流程和测试策略，确保与最新的代码结构和核心流程一致。
